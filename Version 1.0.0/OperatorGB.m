...
Global`OperatorGB::usage="A few definitions need to be made by the user. Type ?SetUpRing, ?Weight for more information.";
...
Weight::usage="A list {w1,w2,...} defining weights on the variables for the weighted DegLex order. Then the weight w1 corresponds to the first variable,
w2 to the second one and so on."
SortedQ::usage="Defines the order of the monomials. DegLex, WeightedDegLex and MultiLex are predefined."
...
(
	If[AllTrue[a,MemberQ[Knowns,#]&] && MemberQ[Map[MemberQ[Unknowns,#]&,b],True],Return[True]];
	If[MemberQ[Map[MemberQ[Unknowns,#]&,a],True] && AllTrue[b,MemberQ[Knowns,#]&],Return[False]];
	Return[DegLex[a,b]];
);
...
t2 = Timing[RewriteGroebner[cofactors,Info->info,OutputProd->outputProd];
		If[outputProd,
			rules = Dispatch[Map[{x_,-#,y_}->{-x,#,y}&,Map[ToProd,ideal]]],
			rules = Dispatch[Map[{x_,-#,y_}->{-x,#,y}&,ideal]]
		];
		cofactors = Map[{#[[1]],#[[2]]/.rules}&,cofactors];
][[1]];
...
Module[{m,lt,poly,i},
	...
	{lt[[2]], Function@@{Evaluate/@m , -1/lt[[1]]*Remainder[poly,lt]}//.Table[lt[[2]][[i]] -> m[[i]],{i,Length[m]}] }
]
...
MultiplyOut[cofactors_List]:=ToNonCommutativeMultiply[Total[Map[ToProd,cofactors]]]
...
Certify[assumptions_List,claims_,Q:Quiver,OptionsPattern[{MaxIter->10,MaxDeg->Infinity,Info->False,Parallel->True,Sorted->False}]]:=
 Module[{reduced,vars,cofactors,G,sigAssump,sigClaim},
	...
	(*and return the reduced claims and the linear combinations*)
	{sigAssump,sigClaim,reduced,Rewrite[vars,cofactors,InputProd->True]}
]
